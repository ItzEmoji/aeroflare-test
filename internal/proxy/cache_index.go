...
func (ci *CacheIndex) refresh(ctx context.Context) error {
	...
	proto := oci.GetProtocol(ci.Registry)

	// 1. Fetch manifest — this is the single source of truth for annotations.
	anns, err := oci.FetchAeroflareAnnotations(ctx, client, ci.Registry, ci.Repository, "cache-config", token)
	if err != nil {
		slog.Error("cache-config failed", "err", err)
		anns, err = oci.FetchAeroflareAnnotations(ctx, client, ci.Registry, ci.Repository, "cache-index", token)
		if err != nil {
	...
	resp, err := oci.DoWithRetry(blobClient, func() (*http.Request, error) {
