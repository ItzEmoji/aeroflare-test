...
func (b *JSONBackend) PushReceipts(ctx context.Context, receipts []cacheindex.PushReceipt) error {
	const maxAttempts = 4
	...
	for attempt := 1; ; attempt++ {
		...
		existingIndex, _, err := cacheindex.FetchCacheIndexWithDigest(b.cfg.Registry, b.cfg.Repository, b.cfg.Token)
		if err != nil {
			return fmt.Errorf("failed to fetch existing index: %w", err)
		}

		writtenDigest, err := cacheindex.UpdateCacheIndexWithDigest(receipts, existingIndex, b.cfg.Registry, b.cfg.Repository, b.cfg.Token, b.cfg.PubKeyPath, b.cfg.ConfigAnnotations, b.cfg.Workers)
		if err != nil {
			return err
		}
		...
		_, currentDigest, err := cacheindex.FetchCacheIndexWithDigest(b.cfg.Registry, b.cfg.Repository, b.cfg.Token)
